...
def infer_batch_builder(data_generator, args):
    st1 = time.time()
    if args.use_gpu:
        paddle.init(use_gpu=True, trainer_count=args.trainer_count, rnn_use_batch=True)
    else:
    ...
    et1 = time.time()
    print("#> Time spent loading model: {}".format(et1 - st1))

    def infer_func(infer_data, codec, batch_id):
        st2 = time.time()
